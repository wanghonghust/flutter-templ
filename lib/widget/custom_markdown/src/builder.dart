...
final List<String> _kTocTags = <String>['h1', 'h2', 'h3', 'h4', 'h5', 'h6'];
...
class MarkdownBuilder implements md.NodeVisitor {
  ...
  Pair<List<Widget>, List<Toc>> build(List<md.Node> nodes) {
    _listIndents.clear();
    _blocks.clear();
    _tables.clear();
    _inlines.clear();
    ...
    return Pair(_blocks.single.children, _tocList);
  }
  ...
  bool visitElementBefore(md.Element element) {
    ...
    int? start;
    if (_isBlockTag(tag)) {
      _addAnonymousBlockIfNeeded(element);
      if (_isListTag(tag)) {
  ...
  void visitElementAfter(md.Element element) {
    ...
    if (_isBlockTag(tag)) {
      ...
      if (_kTocTags.contains(element.tag)) {
        _tocList.add(Toc(
            title: element.textContent,
            level: _kTocTags.indexOf(element.tag),
            index: _blocks.last.children.length));
      }
  ...
  void _addBlockChild(Widget child, md.Element element) {
  ...
  void _addAnonymousBlockIfNeeded(md.Element element) {
    if (_inlines.isEmpty) {
      return;
    }
